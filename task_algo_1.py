...
#  input from user
legth_of_string = int(input('Enter the length  of string : '))
    ...
    try:
        ...
        if len(string) == legth_of_string:
            ...
            if final:
                ...
                print('Sorry we do not find proper result. try again')
        else:
            print('Input binary string length should be {} instead of {}'.format(legth_of_string, len(string)))
    except:
        print('Please give proper binary number input which has only 0 and 1 number')
else:
    print('Please enter string length less than 10^5')
...
n, r, x, y = input("Enter a 4 value of n, r, x, y respectively: ").split()
n,r,x,y = map(int,[n,r,x, y] )
c = input('Enter sequence of integers c1  , ..., c{} : '.format(n))
s = input('Enter sequence of integers s1,..., s{} : '.format(n))
is_contest = c.split()
is_eat = s.split()
new_rating = r
for i in range(n):
